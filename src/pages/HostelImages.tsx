  ...
  return (
    <>
      ...
      <div className="flex flex-col min-h-screen">
        {/* Header with back button - hidden in fullscreen mode */}
        {!fullscreen && (
        ...
        <main className="flex-grow">
          <div className={`${fullscreen ? 'fixed inset-0 bg-black z-50' : 'w-full h-full'}`}>
            {/* Image gallery - Optimized for landscape view */}
            <div 
              className={`relative ${
                fullscreen 
                  ? 'fixed inset-0 flex items-center justify-center bg-black' 
                  : 'w-full flex flex-col items-center justify-center bg-gray-100 md:max-w-5xl md:mx-auto md:rounded-lg md:overflow-hidden'
              }`}
              style={{
                height: fullscreen ? '100%' : 'calc(60vh)',
            ...
            >
              {hostel.images.length > 0 ? (
                <div className="relative w-full h-full flex items-center justify-center">
                  <div className="max-w-full max-h-full flex items-center justify-center">
                    <img 
                      ...
                      className={`${
                        fullscreen 
                          ? 'cursor-zoom-out' 
                          : 'cursor-zoom-in'
                      } shadow-lg rounded-sm w-full h-full object-contain`}
                      style={{ 
                        objectFit: 'contain',
                    ...
                    />
                    {/* Hostel name overlay: always show on fullscreen, and on desktop/tablet (sm+) in non-fullscreen */}
                    {(fullscreen || (!fullscreen && typeof window !== 'undefined' && window.innerWidth >= 640)) && (
                      <div className="absolute bottom-4 left-4 bg-black/60 text-white px-4 py-2 rounded-md text-lg font-semibold z-20 max-w-[90vw] md:max-w-[60vw] truncate shadow-lg">
                        {hostel.name} - {hostel.type}
                      </div>
                    )}
                  </div>
              ...
              {hostel.images.length > 1 && (
                <>
                  <button
                    className="absolute left-2 md:left-6 top-1/2 transform -translate-y-1/2 bg-white/80 hover:bg-white text-[#FF5500] rounded-full w-10 h-10 sm:w-12 sm:h-12 md:w-16 md:h-16 flex items-center justify-center shadow-lg transition-all hover:scale-105 z-10"
                    onClick={handlePrevImage}
                  >
                    <ArrowLeft className="h-5 w-5 sm:h-6 sm:w-6 md:h-8 md:w-8" />
                  </button>
                  <button
                    className="absolute right-2 md:right-6 top-1/2 transform -translate-y-1/2 bg-white/80 hover:bg-white text-[#FF5500] rounded-full w-10 h-10 sm:w-12 sm:h-12 md:w-16 md:h-16 flex items-center justify-center shadow-lg transition-all hover:scale-105 z-10"
            ...
            {!fullscreen && (
              <div className="w-full px-2 bg-white pt-2 pb-4 flex flex-col items-center">
                {/* Image counter */}
                <div className="text-center text-gray-600 py-1 text-sm md:text-base">
                  {currentIndex + 1} / {hostel.images.length}
                </div>
                <div className="grid grid-cols-5 sm:grid-cols-6 md:grid-cols-10 gap-1 sm:gap-2 max-w-5xl w-full">
            ...
            {fullscreen && (
              <div className="absolute bottom-4 left-0 right-0 text-center text-white z-10 text-sm md:text-base">
                {currentIndex + 1} / {hostel.images.length}
              </div>
            )}
          </div>
